...
/*
	...
	Vector range_topo_positions;
	range_topo_positions.x = 1;
	range_topo_positions.y = 2;
	range_topo_positions.z = 3;

	Vector range_topo_velocitys;
	range_topo_velocitys.x = 1;
	range_topo_velocitys.y = 2;
	range_topo_velocitys.z = 3;

	range_topo2look_angles(azimuth, elevation,azimuth_velocity, elevation_velocity,&range_topo_positions, &range_topo_velocitys);

	printf("Entered parameters: azimuth 45, elevation 10, topo_position(1,2,3), topo_velocity(1,2,3)\n");
	printf("The azimuth and elevation velocities are %f and %f.\n",azimuth_velocity,elevation_velocity );
*/
...
//rev/sec
	...
	sat_ECI(&Pos, &Vel, e, ecc_anomaly*PI/180, semi, RAAN*PI/180, w*PI/180, i*PI/180, nt/86400);
...
/*
	...
	char *filename = "newSTKout.e";
	int a = STKout(t, size, filename, coordinatesys, p, v);
